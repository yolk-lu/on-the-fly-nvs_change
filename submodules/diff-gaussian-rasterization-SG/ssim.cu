...
torch::Tensor fused_ssim(torch::Tensor img1, torch::Tensor img2, std::string padding, bool train) {
    double C1 = 0.01 * 0.01;
    double C2 = 0.03 * 0.03;

    assert(std::find(allowed_padding.begin(), allowed_padding.end(), padding) != allowed_padding.end());
