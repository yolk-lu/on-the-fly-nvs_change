...
// ------------------------------------------------------------------------------------
// TODO: Hybrid SH and SG functions, SH(diffuse) + SG(specular)
// change variable : const float* shs -> const float* sg_params
__device__ glm::vec3 computeColorFromHybrid(int idx, const glm::vec3* means, glm::vec3 campos, const float* dc, const float* sg_params, bool* clamped)
{
	// float* dc : SH degree 0 (base color)

	// part 1 : base color
	glm::vec3* direct_color_ptr = ((glm::vec3*)dc) + idx;
	glm::vec3 diffuse_color = SH_C0 * direct_color_ptr[0];
	diffuse_color += 0.5f;
	
	// part 2 : specular color from SG
	...
	// activation part
 	// Normalize axis
	glm::vec3 axis = glm::normalize(axis_raw);

	// sharpness must be positive
	float sharpness = expf(sharpness_raw);

	// amplitude be limited to [0, 1] interval
	glm::vec3 amplitude = {sigmoid(amplitude_raw.x), sigmoid(amplitude_raw.y), sigmoid(amplitude_raw.z)};

	// geometric relationships
	glm::vec3 pos = means[idx];

	// direction from point to camera
	glm::vec3 view_dir = glm::normalize(campos - pos);

	// spherical gaussian formula
	// ampliutude * exp(sharpness(lambda) * (dot(view, axis) - 1))
	
	// dot(view, axis)
	float cos_angle = glm::dot(view_dir, axis);
	cos_angle = fminf(fmaxf(cos_angle, -1.0f), 1.0f); // avoid numerical issues

	// exponent part
	float exponent = sharpness * (cos_angle - 1.0f);
	glm::vec3 specular_color = amplitude * expf(exponent);

	// combine diffuse and specular
	glm::vec3 result = diffuse_color + specular_color;

	// Clamping part - let Backward pass know where gradient should set 0
	clamped[3 * idx + 0] = (result.x < 0);
	clamped[3 * idx + 1] = (result.y < 0);
	clamped[3 * idx + 2] = (result.z < 0);
...
__device__ void computeCov3D(const glm::vec3 scale, float mod, const glm::vec4 rot, float* cov3D) // quats mat3
{
...
// 	depths[idx] = p_view.z;
// 	dists[idx] = glm::length(*(glm::vec3*)(&p_view) - *cam_pos);
// 	radii[idx] = my_radius;
...
// change color precomp part to hybrid SH + SG
	...
	bool prefiltered)
{
	...
	// TODO : change computeColorFromSH -> computeColorFromHybrid
	if (colors_precomp == nullptr)
	{
		glm::vec3 result = computeColorFromHybrid(idx, (glm::vec3*)orig_points, *cam_pos, dc, shs, clamped);
		rgb[idx * C + 0] = result.x;
		rgb[idx * C + 1] = result.y;
		rgb[idx * C + 2] = result.z;
	}
	...
	int* __restrict__ mainGaussID)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			if (best_weight < weight) {
				...
				best_weight = weight;
				best_id = collected_id[j];
				// expected_invdepth = (1.f / depths[collected_id[j]]);
			}
			else if (second_best_weight < weight) {
				second_best_weight = weight;
				second_best_id = collected_id[j];
			}
	...
	if (inside)
	{
		mainGaussID[pix_id] = best_id;
		mainGaussID[H * W + pix_id] = second_best_id;
		final_T[pix_id] = T;
