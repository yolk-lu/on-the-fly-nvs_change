...
// TODO : change to hybrid SH + SG
// params : 
// dL_dcolors : gradient of loss w.r.t. output colors
// dL_dmeans : gradient of loss w.r.t. Gaussian means
// dL_ddc : gradient of loss w.r.t. diffuse 
// SG params : gradient of loss w.r.t. SG params


__device__ void computeColorFromHybridBacward(int idx, const glm::vec3* means, glm::vec3 campos, const float* dc, const float* sg_params, const bool* clamped, const glm::vec3* dL_dcolors, glm::vec3* dL_dmeans, glm::vec3* dL_ddc, float* dL_dsg_params)
{
	...
	glm::vec3 pos = means[idx];
	glm::vec3 view_dir_unorm = campos - pos;
	float dist = glm::length(view_dir_unorm);

	// avoid division by zero
	glm::vec3 view_dir = view_dir_unorm / (dist + 1e-7f);


	// SG params
	const float* p = sg_params + idx * 7;
	glm::vec3 axis_raw = {p[0], p[1], p[2]};
	float sharpness_raw = p[3];
	glm::vec3 amplitude_raw = {p[4], p[5], p[6]};
	
	// activation
	float axis_len = glm::length(axis_raw);
	glm::vec3 axis = axis_raw / (axis_len + 1e-7f); // Normalize

	float sharpness = expf(sharpness_raw);
	glm::vec3 amplitude_sigmoid = {sigmoid(amplitude_raw.x), sigmoid(amplitude_raw.y), sigmoid(amplitude_raw.z)};
	float cos_angle = glm::dot(view_dir, axis);
	float cos_clamped = fminf(fmaxf(cos_angle, -1.0f), 1.0f);

	float exponent = sharpness * (cos_clamped - 1.0f);
	...
	// dL/ddc = dL/dC * SH_C0
	if (dL_ddc) {
		dL_ddc[idx] = dL_dRGB * SH_C0;
	}


	// for amplitude gradient
	// dL/damplitude_raw = dL/dC * exp_val * sigmoid_derivative(amplitude)
	glm::vec3 dL_dAmplitude_raw;
	dL_dAmplitude_raw.x = dL_dRGB.x * exp_val * sigmoid_derivative(amplitude_sigmoid.x);
	dL_dAmplitude_raw.y = dL_dRGB.y * exp_val * sigmoid_derivative(amplitude_sigmoid.y);
	dL_dAmplitude_raw.z = dL_dRGB.z * exp_val * sigmoid_derivative(amplitude_sigmoid.z);


	// for exponent gradient
	// dL/dExp = dot(dL, amplitude) * exp_val
	float dL_dExp = glm::dot(dL_dRGB, amplitude_sigmoid) * exp_val;
	// for sharpness gradient
	// Exp = Sharp * (cos - 1) -> dL/dSharp = dL/dExp * (cos - 1)
	// Sharp = exp(Sharp_raw) -> dSharp/dSharp_raw = Sharp
	float dL_dSharp_raw = dL_dExp * (cos_clamped - 1.0f) * sharpness;

	// for cos_angle gradient
	// dExp/dCos = Sharpness
	float dL_dCos = dL_dExp * sharpness;

	// for axis gradient
	// Cos = dot(view_dir, Axis_norm)
	// dCos/dAxis_norm = view_dir
	glm::vec3 dL_dAxis_norm = dL_dCos * view_dir;


	// Normalize derivative for axis_norm => d/dx (x/|x|) = (I - n * n.T) / |x|
	glm::vec3 dL_dAxis_raw = (dL_dAxis_norm - axis * glm::dot(dL_dAxis_norm, axis)) / (axis_len + 1e-7f); // avoid div by zero

	// for position(means) gradient
	if(dL_dmeans){
		//dCos/dview_dir = axis
		glm::vec3 dL_dview_dir_norm = dL_dCos * axis;

		// normalize derivative for view_dir
		glm::vec3 dL_dview_dir_unorm = (dL_dview_dir_norm - view_dir * glm::dot(dL_dview_dir_norm, view_dir)) / (dist + 1e-7f);

		// view_dir_unorm = campos - pos => d/dpos = -1
		glm::vec3 dL_dPos = -dL_dview_dir_unorm;

		atomicAdd(&(dL_dmeans[idx].x), dL_dPos.x);
		atomicAdd(&(dL_dmeans[idx].y), dL_dPos.y);
		atomicAdd(&(dL_dmeans[idx].z), dL_dPos.z);
	}

	// write gradients to SG params = [axis_x, axis_y, axis_z, sharpness, amplitude_r, amplitude_g, amplitude_b]
	float* dL_dp = dL_dsg_params + idx * 7;
	dL_dp[0] = dL_dAxis_raw.x;
	dL_dp[1] = dL_dAxis_raw.y;
	dL_dp[2] = dL_dAxis_raw.z;
	dL_dp[3] = dL_dSharp_raw;
	dL_dp[4] = dL_dAmplitude_raw.x;
	dL_dp[5] = dL_dAmplitude_raw.y;
	dL_dp[6] = dL_dAmplitude_raw.z;
	...
	float* dL_dopacity)
{
	...
	// TODO: change to Hybrid mode

	if(shs)
		computeColorFromHybridBacward(
									idx,
									(glm::vec3*)means,
									*campos, 
									dc, 
