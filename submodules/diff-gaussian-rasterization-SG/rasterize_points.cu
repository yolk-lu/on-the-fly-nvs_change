	...
	const bool debug)
{
  ...
  torch::Tensor mainGaussID = torch::full({2, H, W}, 0.0, int_opts);
  ...
  if(P != 0)
  {
	  ...
	  auto tup = CudaRasterizer::Rasterizer::forward(
		...
		mainGaussID.contiguous().data<int>(),
  ...
  return std::make_tuple(rendered, num_buckets, out_color, out_invdepth, mainGaussID, radii, geomBuffer, binningBuffer, imgBuffer, sampleBuffer);
}
...
// TODO : change the memory allocation for SH and Hybrid
  torch::Tensor dL_dsh;
  if (sh.ndimension() == 2){
	//sg mode
  ...
  torch::Tensor dL_dt = torch::zeros({P, 3}, means3D.options()); // quats {P, 3, 3}
  ...
  {  
	  ...
	  dL_dRs.contiguous().data<float>(),
	  dL_dt.contiguous().data<float>(),
	  debug);
  }

  torch::Tensor dL_dRt = torch::zeros({4, 4}, means3D.options());
  dL_dRs.nan_to_num_();
  dL_dt.nan_to_num_();
  dL_dRt.index_put_({Slice(0, 3), Slice(0, 3)}, dL_dRs.sum(0));
  dL_dRt.index_put_({3, Slice(0, 3)}, dL_dt.sum(0));
  return std::make_tuple(dL_dmeans2D, dL_dcolors, dL_dopacity, dL_dmeans3D, dL_dcov3D, dL_ddc, dL_dsh, dL_dscales, dL_drotations, dL_dRt);
}
