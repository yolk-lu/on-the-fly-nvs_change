...
if __name__ == "__main__":
            ...
            if args.display_runtimes:
                ...
                for key, value in tracker.stats.items():
                    if value["count"] > 0:
                        ...
                        bar_postfix += [
                            f"\033[35m{key}:{1000 * value['time'] / value['count']:.1f}\033[0m"
                        ]
            bar_postfix += [
    ...
    if len(args.save_at_finetune_epoch) > 0:
        ...
        finetune_epochs_per_level = max(args.save_at_finetune_epoch)
    elif args.lod_max > args.lod_min:
        finetune_epochs_per_level = 10 # Default epochs per level if LoD is used but no explicit epochs set

    current_lod_step = scene_model.current_lod
    # If we haven't started LoD loop yet (e.g. init reconstruction was lod_min)

    while current_lod_step <= args.lod_max:
        if finetune_epochs_per_level > 0:
            print(f"\n--- Training LoD {current_lod_step} ---")
        ...
        # pbar = tqdm(range(0, finetune_epochs), desc="Fine tuning")
            ...
            # if epoch + 1 in args.save_at_finetune_epoch    
                # Save intermediate models within the level if requested
                if (epoch + 1) in args.save_at_finetune_epoch:
                    torch.cuda.empty_cache()
                    scene_model.inference_mode = True
                    metrics = scene_model.save(
                        # os.path.join(args.model_path, str(epoch + 1)), reconstruction_time
                        os.path.join(args.model_path, f"lod_{current_lod_step}_epoch_{epoch + 1}"), reconstruction_time
                    )
                    bar_postfix = []
                    for key, value in metrics.items():
                        bar_postfix += [f"\033[31m{key}:{value:.2f}\033[0m"]
        ...
        if current_lod_step < args.lod_max:
            ...
            current_lod_step = scene_model.current_lod # should be incremented
        else:
            break
